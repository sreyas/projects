...
(function($) {
    ...
    $.fn.editableSelect = function(options) {
        ...
        $(this).each(function(){
            ...
            html += (opts.editable ?
                '<div class="inputbox"><input type="text" placeholder="Enter New, Press Enter, click Save" /></div>' : ''); //<button type="button">+</button>
            ...
            if(opts.editable) {
                ...
                $input.bind('keypress',function(event){
                    if(event.keyCode == "13"){
                        ...
                        // Custom code to implement insert to table    
                        var fieldvalue = text;
                        var fieldname = $(this).closest('.ui-select-wrap').siblings('.editable-select').attr('name');
                        var url = "";
                        var param = "";
                        ...
                        switch(fieldname) {
                            ...
                            case 'permpl.Client':
                            case 'temppl.Client':                            
                                url = '/clients/saveajax'; 
                                break;
                            case 'Sourcer' : 
                            case 'permpl.Sourcer':
                            case 'temppl.Sourcer':                            
                                url = '/sourcers/saveajax'; 
                                break;
                            case 'Clientrep' : 
                            case 'permpl.Clientrep':
                            case 'temppl.Clientrep':                            
                                url = '/clientreps/saveajax'; 
                                break;
                            default : 
                                url = ''; 
                                break;
                        ...
                        if((fieldvalue.length) > 0) {
                            var saveData = $.ajax({
                                  type: "POST",
                                  url: url,
                                  data: { "ajaxfield" : fieldvalue },
                                  success: function(result) {  

                                    var resobj = JSON.parse(JSON.stringify(result));
                                    val = resobj.data.InsertID;       
